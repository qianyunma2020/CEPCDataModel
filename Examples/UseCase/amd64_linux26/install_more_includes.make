...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "UseCase" = ""; then \
	    if test -d ../UseCase; then \
	      echo "Installing files from more ../UseCase to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/UseCase; then mkdir -p $(install_include_dir)/UseCase; fi;\
	        $(install_command) ../UseCase $(install_include_dir);\
	    else \
	      echo "No more UseCase include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/UseCase"; \
	  $(uninstall_command) "$(install_include_dir)/UseCase/*" ; \
	  if test -f $(install_include_dir)/UseCase.cmtref ; then \
	    echo "Removing $(install_include_dir)/UseCase.cmtref"; \
	    eval rm -f $(install_include_dir)/UseCase.cmtref ; \
	  fi; \
	fi
